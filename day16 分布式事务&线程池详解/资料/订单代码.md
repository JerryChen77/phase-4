...
public class OrderServiceImpl extends ServersManager<Order, OrderDao> implements
	...
	public Order createMiaoshaOrder(Order order) {
        ...
        //TODO 插入日志 log(orderId，productId，deductStockCount，status...) 当把订单信息放入到日志中以后，下订单就成功了。那么减库存和加积分怎么去实现呢？
        
        //定时任务 轮询执行程序，从日志里获取该订单信息，往库存库里操作，如果插入成功，定时任务就不需要继续执行，如果不成功，继续执行。如果失败，开启重试，如果重试达到阈值，人工介入。
